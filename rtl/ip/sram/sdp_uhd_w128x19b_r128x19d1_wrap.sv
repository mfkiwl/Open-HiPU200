...
// ---------------------------------------------------------------------------------------------------------------------
// FILE NAME  : sdp_uhd_w128x19b_r128x19d1_wrap.sv
...
module sdp_uhd_w128x19b_r128x19d1_wrap (
    input           clk_i,
    input           we_i,
    input [6:0]     waddr_i,
    input [18:0]    wdata_i,
    input [18:0]    wdata_bwe_i,
    input           re_i,
    input [6:0]     raddr_i,
    output[18:0]    rdata_o
);
...
`ifdef PLATFORM_XILINX
    logic[18 : 0]   bwebb;
    logic           webb;
    logic[18 : 0]   wmask_dly1;
    logic[18 : 0]   wdata_dly1, qa;

    assign bwebb = ~wdata_bwe_i;

    assign webb = ~we_i;

    for(genvar gi = 0; gi < 19; gi=gi+1) begin
    ...
    sdp_w128x19_r128x19d1 sdp_w128x19_r128x19d1_inst (
        .clka           (clk_i),
        .ena            (1'b1),
        .wea            ({3{we_i}}),
        .addra          (waddr_i),
...
`ifdef PLATFORM_ASIC
    ...
    TSDN28HPCPUHDB128X19M4MW sdp_w128x19_r128x19d1_inst (
        // mode
        //.AWT            (1'b0),
        //.VDD            (1'b1),
        //.VSS            (1'b0),
...
endmodule : sdp_uhd_w128x19b_r128x19d1_wrap
