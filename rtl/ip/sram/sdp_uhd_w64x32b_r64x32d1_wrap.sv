...
// ---------------------------------------------------------------------------------------------------------------------
// FILE NAME  : sdp_uhd_w64x32b_r64x32d1_wrap.sv
...
module sdp_uhd_w64x32b_r64x32d1_wrap (
    input           clk_i,
    input           we_i,
    input [5:0]     waddr_i,
    input [31:0]    wdata_i,
    input [31:0]    wdata_bwe_i,
    input           re_i,
    input [5:0]     raddr_i,
    output[31:0]    rdata_o
);
...
`ifdef PLATFORM_XILINX
    logic[31 : 0]   bwebb;
    logic           webb;
    logic[31 : 0]   wmask_dly1;
    logic[31 : 0]   wdata_dly1, qa;

    assign bwebb = ~wdata_bwe_i;

    assign webb = ~we_i;

    for(genvar gi = 0; gi < 32; gi=gi+1) begin
    ...
    sdp_w64x32_r64x32d1 sdp_w64x32_r64x32d1_inst (
        .clka           (clk_i),
        .ena            (1'b1),
        .wea            ({4{we_i}}),
        .addra          (waddr_i),
...
`ifdef PLATFORM_ASIC
    ...
    TSDN28HPCPUHDB64X32M4MW sdp_w64x32_r64x32d1_inst (
        // mode
        //.AWT            (1'b0),
        //.VDD            (1'b1),
        //.VSS            (1'b0),
...
endmodule : sdp_uhd_w64x32b_r64x32d1_wrap
